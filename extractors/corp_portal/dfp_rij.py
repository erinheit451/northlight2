...
from .portal_selectors import DFP_RIJ_URL, R_RUN_REPORT_SELECTORS, RESULTS_READY_MARKERS
...
def run():
    ...
    try:
        # 1) Fresh login and navigate to DFP-RIJ report
        print("[DEBUG] Starting authentication...")
        ensure_logged_in(page)
        print("[DEBUG] Authentication completed, navigating to DFP-RIJ report...")
        page.goto(DFP_RIJ_URL, wait_until="domcontentloaded")
        try:
            page.wait_for_load_state("networkidle", timeout=30_000)
        except:
            print("[DEBUG] Network idle timeout, but continuing...")
        print(f"[DEBUG] DFP-RIJ report loaded, current URL: {page.url}")
        ...
        if not _wait_results_ready(page):
            ...
            if not run_btn:
                ...
                page.screenshot(path="logs/dfp_rij_no_run.png", full_page=True)
                with open("logs/dfp_rij_dom_before_run.html", "w", encoding="utf-8") as f:
                    f.write(page.content())
                with open("logs/dfp_rij_frames_before_run.txt", "w", encoding="utf-8") as f:
                    for fr in page.frames:
            ...
            if not _wait_results_ready(page):
                ...
                if not _wait_results_ready(page):
                    page.screenshot(path="logs/dfp_rij_after_run_no_results.png", full_page=True)
                    with open("logs/dfp_rij_dom_after_run.html", "w", encoding="utf-8") as f:
                        f.write(page.content())
                    raise RuntimeError("Ran the DFP-RIJ report but did not detect results/controls.")
        ...
        if not export_link:
            page.screenshot(path="logs/dfp_rij_no_export.png", full_page=True)
            with open("logs/dfp_rij_dom_no_export.html", "w", encoding="utf-8") as f:
                f.write(page.content())
            raise RuntimeError('Export link not found after results. Check logs/dfp_rij_dom_no_export.html')
        ...
        suggested = dl.suggested_filename or f"dfp_rij_{_date_stamp()}.xls"
        ...
        final_path = os.path.join(download_dir_abs, f"dfp_rij_{_date_stamp()}{ext}")
        ...
        if _looks_like_login(head):
            page.screenshot(path="logs/dfp_rij_login_after_download.png", full_page=True)
            with open("logs/dfp_rij_after_download.html", "w", encoding="utf-8") as f:
                f.write(page.content())
            raise RuntimeError("Downloaded login HTML instead of data (auth bounce).")

        print(f"[OK] DFP-RIJ exported to {final_path}")
        return True

    except Exception as e:
        print(f"[ERROR] DFP-RIJ extraction failed: {e}")
        return False
    finally:
        context.close()
        pw.stop()
