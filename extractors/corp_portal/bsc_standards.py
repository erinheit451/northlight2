...
from .portal_selectors import BSC_STANDARDS_URL, R_RUN_REPORT_SELECTORS, RESULTS_READY_MARKERS, select_partner_channel
...
def run():
    ...
    try:
        # 1) Fresh login and navigate to BSC Standards report
        print("[DEBUG] Starting authentication...")
        ensure_logged_in(page)
        print("[DEBUG] Authentication completed, navigating to BSC Standards report...")
        page.goto(BSC_STANDARDS_URL, wait_until="domcontentloaded")
        try:
            page.wait_for_load_state("networkidle", timeout=30_000)
        except:
            print("[DEBUG] Network idle timeout, but continuing...")
        print(f"[DEBUG] BSC Standards report loaded, current URL: {page.url}")
        ...
        if not _wait_results_ready(page):
            ...
            if not run_btn:
                ...
                page.screenshot(path="logs/bsc_standards_no_run.png", full_page=True)
                with open("logs/bsc_standards_dom_before_run.html", "w", encoding="utf-8") as f:
                    f.write(page.content())
                with open("logs/bsc_standards_frames_before_run.txt", "w", encoding="utf-8") as f:
                    for fr in page.frames:
            ...
            if not _wait_results_ready(page):
                ...
                if not _wait_results_ready(page):
                    page.screenshot(path="logs/bsc_standards_after_run_no_results.png", full_page=True)
                    with open("logs/bsc_standards_dom_after_run.html", "w", encoding="utf-8") as f:
                        f.write(page.content())
                    raise RuntimeError("Ran the BSC Standards report but did not detect results/controls.")
        ...
        if not export_link:
            page.screenshot(path="logs/bsc_standards_no_export.png", full_page=True)
            with open("logs/bsc_standards_dom_no_export.html", "w", encoding="utf-8") as f:
                f.write(page.content())
            raise RuntimeError('Export link not found after results. Check logs/bsc_standards_dom_no_export.html')
        ...
        suggested = dl.suggested_filename or f"bsc_standards_{_date_stamp()}.xls"
        ...
        final_path = os.path.join(download_dir_abs, f"bsc_standards_{_date_stamp()}{ext}")
        ...
        if _looks_like_login(head):
            page.screenshot(path="logs/bsc_standards_login_after_download.png", full_page=True)
            with open("logs/bsc_standards_after_download.html", "w", encoding="utf-8") as f:
                f.write(page.content())
            raise RuntimeError("Downloaded login HTML instead of data (auth bounce).")

        print(f"[OK] BSC Standards exported to {final_path}")
        return True

    except Exception as e:
        print(f"[ERROR] BSC Standards extraction failed: {e}")
        return False
    finally:
        context.close()
        pw.stop()
